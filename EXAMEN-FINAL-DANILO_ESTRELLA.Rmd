---
title: "Discurso presidencial del 1996"
output: 
...
library(pacman)
p_load(flexdashboard, gganimate, dplyr, pdftools, stringr, readxl, tidyverse, tidytext, tm, wordcloud, wordcloud2, patchwork, widyr, magrittr, igraph, ggraph, grid, extrafont, RcolorBrewer, ggplot2, plotly, scales, gifski, remotes, crosstalk, DT, d3scatter, kableExtra, knitr, rsconnect)
```  
...
Observando los gráficos podemos ver que el discurso del expresidente Alberto Fujimori trata sobre el desarrollo del país, también habla del congreso y del diñero invertido.
...
```{r nube de palabras}
top_palabras <- discurso_fujimori_frecuencias %>%
                 top_n(30, n)

# Crear la nube de palabras en forma de círculo
wordcloud(words = top_palabras$Token,      # Palabras
          freq = top_palabras$n,            # Frecuencias
          min.freq = 1,                     # Frecuencia mínima
          max.words = 30,                   # Máximo de palabras a mostrar
          random.order = FALSE,             # Orden aleatorio desactivado
          colors = brewer.pal(8, "Accent"),  # Colores
